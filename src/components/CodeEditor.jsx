...
export default function CodeEditor({ initialCode = "", onRun, loading = false, onChangeCode, onGetHint, hintLoading = false }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-[linear-gradient(135deg,rgba(0,224,192,0.07),rgba(192,0,144,0.04))] border border-cyan-400/20 shadow-[0_0_10px_#00E0C099]">
        ...
        <div className="flex items-center gap-2">
          {onGetHint && (
            <button
              ...
              className="px-3 py-1 bg-[linear-gradient(to_right,#6EE7B7,#C00090)] hover:scale-105 text-white rounded-lg text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_6px_#00E0C099]"
            >
      ...
      <textarea
        value={code}
        onChange={(e) => { const v = e.target.value; setCode(v); if (onChangeCode) onChangeCode(v); }}
